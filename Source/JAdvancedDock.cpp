...
public:
	...
	void resized() override
	{
		...
		topArea = area.removeFromTop(h3).translated(w3, 0.0f).withWidth(w3);
		...
		bottomArea = area.translated(w3, 0.0f).withWidth(w3);
...
void JAdvancedDock::addComponentToDock(Component* comp, int row , int col)
{
    auto dockable = manager.createDockableComponent(comp);
    addAndMakeVisible(dockable);
    auto loc = WindowLocation{row, 0, 0};
